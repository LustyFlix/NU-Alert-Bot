    ...
    if req_result['children']['titles'] != []:
        ...
        for i in range(len(req_result['children']['titles'])):
            try:
                ...
                children = ''
    broadcast_msg = "📬 __[{}]: NU Site has been Updated!__\n\n🌀 __Latest Notice Title__ - \n**{}**\n\n{}📍__Under Tab__ --> {}\n\n🔮 Cheers from @NUAlertBot!".format(
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        req_result["title"],
        children,
        req_result["tab"],
